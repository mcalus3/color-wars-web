...
export function trimPointToBoard(
  point: Point,
  dim: Point,
  precision: number
): Point {
...
export function getFirstPointOfAttack(
  territory: Point[],
  coords: Point,
  size: number
): Point {
...
export function getDirectionToAdjecentEnemyField(
  coords: Point,
  territory: Point[]
) {
  ...
  for (let i = 0; i < dirs.length; i++) {
    ...
    if (
      territory.find(p => {
        return PointsAreEqual(p, addPoints(coords, vector));
      }) === undefined
    ) {
      return dirs[i];
    }
...
export function pointsAreClose(
  ...
  return (
    Math.max(
      Math.abs(currentPoint.X - target.X),
      Math.abs(currentPoint.Y - target.Y)
    ) <= precision
  );
}
...
export function getTerritoryPoints(
  color: number,
  colorsArr: number[][]
): Point[] {
  let points: Point[] = [];
...
export function getRandomClosePointFromArray(
  point: Point,
  arr: Point[],
  range: number
): Point {
  let filteredPoints = arr.filter(p => {
    return Math.abs(p.X - point.X) < range && Math.abs(p.Y - point.Y) < range;
  });

  let randomPoint =
    filteredPoints[Math.floor(Math.random() * filteredPoints.length)];

  return { ...randomPoint };
}

export function fieldShouldBeClaimed(
  field: Point,
  colors: number[][],
  dim: Point,
  color: number
