...
export function dispatchAiActions() {
  ...
  for (let i = 0; i < Ais.length; i++) {
    ...
    if (
      newAi.currentDirection !== oldAi.currentDirection ||
      newAi.currentDirection !== state.playersById[i].direction
    ) {
      store.dispatch(
        actions.changeDirection(newAi.playerId, newAi.currentDirection)
      );
    }
...
function updateAiStates(state: GameState) {
  ...
  for (let i = 0; i < players.length; i++) {
    if (
      players[i].aiControlled &&
      Ais[i] === undefined &&
      i < state.activePlayers
    ) {
  ...
  for (let i = 0; i < Ais.length; i++) {
    ...
    if (lastState.playersById[i] !== undefined) {
      Ais[i].PlayerHasDied =
        players[i].deaths !== lastState.playersById[i].deaths;
    }
  }
...
function updateAiTerritories(state: GameState) {
  for (let i = 0; i < Ais.length; i++) {
    ...
    let territoryBorder: Point[] = shuffle(
      getBorder(state.fieldColors, color, state.dimension)
    );
    let territory: Point[] = shuffle(
      getTerritoryPoints(color, state.fieldColors)
    );
...
function createNewAi(state: GameState, id: number, aiRefreshTime: number): Ai {
  return {
    playerId: id,
    territoryBorder: getBorder(
      state.fieldColors,
      state.playersById[id].color,
      state.dimension
    ),
    stepsDone: 0,
    currentTargets: [],
    territory: getTerritoryPoints(
      state.playersById[id].color,
      state.fieldColors
    ),
    currentAction: 'inside',
    currentDirection: 'none',
    precision: calculatePrecision(state.playersById[id].speed, aiRefreshTime),
    PlayerHasDied: false,
