...
export function claimTerritoryAlgorithm(tail: Point[], fields: number[][], color: number): Point[]{
    if(tail.length === 0){
      return [];
    }
    ...
    for(let i: number = 0; i < tail.length; i++){        
      ...
      for(let j: number = 0; j < neigh.length; j++){
        if(!ArrayHasCoords(neigh[j], claimedFields) && !ArrayHasCoords(neigh[j], tailNeighbours))
        {
    ...
    for(let i: number = 0; i < tailNeighbours.length; i++){
        ...
        if(!ArrayHasCoords(tailNeighbours[i], claimedFields) && !ArrayHasCoords(tailNeighbours[i], outerNeighbours))
        {
            ...
            var joinedAreaIsInside: boolean = ClaimTerritoryRecursive(tailNeighbours[i], outerNeighbours, joinedArea, claimedFields, dimension);
  ...
  function ClaimTerritoryRecursive(currentField: Point, outerNeighbours: Point[], joinedArea: Point[], claimedFields: Point[], dimension: Point): boolean{
    ...
    for(let i: number = 0; i < currentNeighbors.length; i++){
      ...
      if((!ArrayHasCoords(currentNeighbors[i], claimedFields)) && (!ArrayHasCoords(currentNeighbors[i], joinedArea)))
      {
          if(!ClaimTerritoryRecursive(currentNeighbors[i], outerNeighbours, joinedArea, claimedFields, dimension))
          {
