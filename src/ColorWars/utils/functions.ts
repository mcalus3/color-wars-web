...
export function deepCopy(obj: any) {
    ...
    if (obj instanceof Date)
      var temp: any = new Date(obj) as any;
    else
      var temp: any = obj.constructor();
...
export function layouter(boardDimension: Point,
...
export function createHistogram(fields: number[][]): {colorsArr: number[], valuesArr: number[]}{
    ...
    var colorsArr: number[] = Object.keys(colors).map(n => parseInt(n));
    colorsArr.sort(function(a: any, b: any) {
      return parseFloat(colors[b]) - parseFloat(colors[a]);
    });
    var valuesArr = colorsArr.map((color: any) => colors[color]);
    return {colorsArr, valuesArr};
  }

export function outOfBoard(coords: Point, dimension: Point): boolean {
  return coords.X < 0 || coords.Y < 0 || coords.X >= dimension.X || coords.Y >= dimension.Y;
}
