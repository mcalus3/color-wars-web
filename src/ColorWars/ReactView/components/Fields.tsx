...
import { colorNumToName, layouter, PointsAreEqual } from '../../utils/functions';
...
class Fields extends Component<Props, object>{
  ...
  drawFields(){
    ...
    let layer = this.refs.layer as any;
    let canvas = layer.canvas._canvas as any;
    if(canvas.width !== this.canvasDim.X || canvas.height !== this.canvasDim.Y){
      this.redraw = true;
    }
    ...
    let color: number = fields[this.lastUpdatedCoords[0].X][this.lastUpdatedCoords[0].Y];
  ...
  drawRectangle(fieldLocation: Point, ctx: CanvasRenderingContext2D, canvasDimension: Point) {
    var X, Y, Width, Height;
    ({X, Y, Width, Height} = layouter(this.props.dimension, canvasDimension, fieldLocation)); 
