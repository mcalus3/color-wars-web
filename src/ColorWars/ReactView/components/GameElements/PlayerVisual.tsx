...
import { colorNumToName, FRAMES_PER_SEC, layouter, swap } from '../../../utils/functions';
...
class PlayerComponent extends React.Component<Props, {image: HTMLImageElement}> {
  ...
  renderKeys(){
    ...
    let keys = swap(this.props.keys);
    if (!(this.props.player.direction === 'none')){
      return null;
    }
    ...
    let upProps: any = {
      x: this.rectangle.X,
      y: this.rectangle.Y - this.rectangle.Width,
      align: 'center'
    }
