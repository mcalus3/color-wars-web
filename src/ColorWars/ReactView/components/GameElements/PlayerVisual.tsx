...
class PlayerComponent extends React.Component<Props, object>{
  ...
  shouldComponentUpdate(nextProps: Readonly<Props>, nextState: object){
    if (nextProps.player.coords.X === this.props.player.coords.X && nextProps.player.coords.Y === this.props.player.coords.Y){
       return false;
     }
     return true;
  }
  ...
  render(){
    if (this.canvas === undefined){
      ...
      var canvasDimension: Point = {X:this.canvas.width,Y:this.canvas.height}      
    }
  
    let color: string = colorNumToName[this.props.player.color];
    this.rectangle = layouter(this.props.dim, canvasDimension, this.props.player.coords);
